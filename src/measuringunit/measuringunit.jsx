  ...
  const onChangeInput = (item, e) => {
    ...
    if (item.isNew) {
      setNewAttributeList((prevList) => {
        ...
        const newAttribute = existingItem
          ? { ...existingItem, [name]: value }
          : { id: item.id, [name]: value };
        return [...prevList.filter((p) => p.id !== item.id), newAttribute];
      });
    }
    const updatedMeasureunits = measureunits.map((mu) =>
      mu.id === item.id ? { ...mu, [name]: value } : mu
    );
    ...
    try {
      if (newAttributeList.length == 1) {
        for (let i of newAttributeList) {
          const result = await createMeaureUnit({
            unit: i.unit,
