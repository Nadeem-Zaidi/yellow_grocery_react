...
const addProduct = async (data) => {
    try {
        const response = await fetch("http://localhost:3001/grocery/product/createProduct",
            {
                method: "POST",
                headers: {
...
export const fetchProducts = async () => {
    try {
        const response = await fetch("http://localhost:3001/grocery/product")
...
export const fetchProduct = async (id) => {
    try {
        const response = await fetch(`http://localhost:3001/grocery/product/${id}`)
        if (!response.ok) {
...
export const fetchProductVariation = async (id) => {
    try {
        const response = await fetch(`http://localhost:3001/grocery/product/variations/${id}`)
        if (!response.ok) {
...
export const fetchProductVariationById = async (id) => {
    try {
        const response = await fetch(`http://localhost:3001/grocery/product/variationbyid/${id}`)
        if (!response.ok) {
            throw new Error("Error in fetching productVariation")
        }
        const result = await response.json()
...
export const addProductVariation = async (data) => {
    try {
        const response = await fetch("http://localhost:3001/grocery/product/createVariation", {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
const addVariationSpec = async () => {
    const response = await fetch("http://localhost:3001/grocery/")
