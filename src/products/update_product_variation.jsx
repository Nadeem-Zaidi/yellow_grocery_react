...
import { addProductVariation, fetchProductVariationById } from "./product_sideeffects/product_side_effects";

const UpdateProductVariation = () => {
    const { id } = useParams();

    const { data: variationDetails, status } = useQuery(["productVariationById", id], () => fetchProductVariationById(id));
    ...
    const variationDescriptionController = (e) => setVariationDescription(e.target.value);
    ...
    return (
        <div className="max-w-4xl mx-auto p-8 bg-white shadow-lg rounded-lg">
            <h2 className="text-2xl font-bold mb-6 text-gray-800">Product Variation Details</h2>
            ...
            <h3 className="text-xl font-bold mt-10 mb-4 text-gray-800">Specifications</h3>
            <table className="min-w-full bg-white shadow-md rounded-lg">
                <thead>
                    <tr className="bg-gray-100 border-b">
                        <th className="py-2 px-4 text-left text-gray-600">Attribute Name</th>
                        ...
                        <tr key={spec.id} className="border-b">
                            ...
                            <td className="py-2 px-4">
                                <input
                                    ...
                                    className={`w-full px-3 py-1 border border-gray-300 rounded-md ${spec.attribute.datatype !== 'ALN' ? 'bg-gray-100 text-gray-700' : 'bg-white'}`}
                                />
                            </td>
                            <td className="py-2 px-4">
                                <input
                                    ...
                                    className={`w-full px-3 py-1 border border-gray-300 rounded-md ${spec.attribute.datatype !== 'NUMERIC' ? 'bg-gray-100 text-gray-700' : 'bg-white'}`}
                                />
                            </td>
                        </tr>
                    ))}
