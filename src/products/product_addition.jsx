...
import ProgressIndicator from "../components/modal/progress_indicator";
...
const AddProduct = () => {
    ...
    const formController = async (e) => {
        ...
        try {
            ...
            setProduct(result);
            setIsLoading(false);
            alert(`Product created with name ${result.name}`);
        } catch (e) {
            setIsLoading(false);
            setError(e);
        }
    ...
    return (
        <form onSubmit={formController} className="p-6 bg-white rounded-lg">
            {categoryLookup && <Modal onClose={closeCategoryLookup} >
                <CategoryList lookUp={true} selectCategory={selectedCategoryPath} />
            </Modal>}
